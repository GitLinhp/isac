...
class blk(gr.basic_block):
    ...
    def _resolve_from_config(config_file: str) -> tuple[str, float]:
        cache_dir = resolve_source_cache_file(
            config_file, _RESOLVE_DEVICE, _RESOLVE_SEED
        )
        samp_rate = float(
            resolve_ofdm_samp_rate(config_file, _RESOLVE_DEVICE, _RESOLVE_SEED)
        )
