...
class usrp_ofdm_burst_tr(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, address="type=x4xx,mgmt_addr=192.168.1.100,addr=192.168.10.2", freq=6.0e9):
        ...
        self.samp_rate = samp_rate = resolve_ofdm_samp_rate(config_file, device, 42)
        self.ofdm_burst_samples = ofdm_burst_samples = resolve_ofdm_burst_len(config_file, device, 42)
        self.idle_ms = idle_ms = 400
        self.gui_update_time_ms = gui_update_time_ms = 10
        self.freq_trig_level = freq_trig_level = -90
        self.dd_vlen = dd_vlen = resolve_dd_output_vlen(config_file, device, 42)
        ...
        self.ofdm_burst_sensing_rx = ofdm_burst_sensing_rx.blk(config_file=config_file, device=device, seed=42, corr_threshold=0.3, template_len=0, idle_ms=idle_ms, dd_vlen=dd_vlen, debug=False)
    ...
    def set_device(self, device):
        self.device = device
        self.set_dd_vlen(resolve_dd_output_vlen(self.config_file, self.device, 42))
        self.set_ofdm_burst_samples(resolve_ofdm_burst_len(self.config_file, self.device, 42))
        self.set_samp_rate(resolve_ofdm_samp_rate(self.config_file, self.device, 42))
