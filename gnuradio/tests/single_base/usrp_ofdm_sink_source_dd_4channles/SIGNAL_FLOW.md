...
本流图实现**零多普勒 OFDM 单站雷达**（环回 / 近距目标），**ch0–ch3 同波四发 / 四收**：
...
4. 四路 `OfdmBurstRx` 各自做延迟对齐 + CUDA 距离谱 + plot/MUSIC。

核心块：`OfdmBurstTxSource`（单口）+ 四个 `OfdmBurstRx` 实例；UHD `channels=[0, 1, 2, 3]`。
...
flowchart LR
  ...
  TxSrc -->|"out0 tee"| Sink
  TxSrc -->|"msg tx_freq_cpi / tx_schedule"| Rx0
  TxSrc -->|"msg"| Rx1
  TxSrc -->|"msg"| Rx2
  TxSrc -->|"msg"| Rx3
  Source -->|"port0"| Rx0
...
| `OfdmBurstTxSource.tx_freq_cpi` | 四个 `OfdmBurstRx`          | PMT（首 CPI 一次；各 RX 粘性复用）           |
| `OfdmBurstTxSource.tx_schedule` | 四个 `OfdmBurstRx`          | PMT（每 CPI）                                |
| `uhd_usrp_source_0:0..3`        | `ofdm_burst_rx_0..3`        | 对应通道接收 IQ                              |
...
| `tx_freq_cpi`   | PMT `(dict meta, c32vector)`          | 四路 RX（首 CPI 一次）     |
| `tx_schedule`   | PMT`(uint64 sec, double frac)`    | 四路 RX 每 CPI             |
...
| 行为 | 按`tx_time`（通道 0 标签）在计划绝对时刻四通道发射 |
...
| 通道 | `channels=[0, 1, 2, 3]` → 四路流口               |
| 格式 | `fc32`                                           |
| 天线 | 流 0–3 均为 `RX1`                                |
| 启动 | 已 patch：不连续流；由 TX **异步** `issue_stream_cmd` 驱动整条多通道流 |
| 其它 | `max_noutput ≥ max(16384, burst_len)`；`min_output_buffer ≥ 2·burst` |

### 7.2 `OfdmBurstRxBlock` ×4（ch0–ch3 各一实例）

每个实例 `out_sig=None`：无流输出。内嵌 `_RangeProfileDisplay`（`plot_title=Range Profile chN`）与可选 `RangeMusicEstimator`。
