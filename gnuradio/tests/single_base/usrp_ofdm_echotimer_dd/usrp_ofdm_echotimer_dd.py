...
# Description: USRP Sink/Source (Echotimer replacement) OFDM radar zero-Doppler range profile. Scheduled RX (NUM_SAMPS_AND_DONE per CPI); packet_len via packet_len_tagger.
...
import sip
import threading
import usrp_ofdm_echotimer_dd_echotimer_rx_compensator_0 as echotimer_rx_compensator_0  # embedded python block
import usrp_ofdm_echotimer_dd_burst_iq_tag_tx_0 as burst_iq_tag_tx_0  # embedded python block
import usrp_ofdm_echotimer_dd_cpi_rate_probe_0 as cpi_rate_probe_0  # embedded python block
import usrp_ofdm_echotimer_dd_ofdm_range_profile_0 as ofdm_range_profile_0  # embedded python block
import usrp_ofdm_echotimer_dd_packet_len_tagger_0 as packet_len_tagger_0  # embedded python block
import usrp_ofdm_echotimer_dd_range_music_block_dev1 as range_music_block_dev1  # embedded python block
import usrp_ofdm_echotimer_dd_range_profile_plot_dev1 as range_profile_plot_dev1  # embedded python block
...
class usrp_ofdm_echotimer_dd(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, address="type=x4xx,serial=349B642,mgmt_addr=192.168.1.100,addr=192.168.10.2,clock_source=internal,time_source=internal"):
        ...
        self.echotimer_rx_compensator_0.set_min_output_buffer(burst_len_samples)
        self.echotimer_rx_compensator_0.set_min_output_buffer(burst_len_samples)
        ...
        self.cpi_rate_probe_0 = cpi_rate_probe_0.CpiRateProbeBlock(length_tag_key=length_tag_key, log_interval_s=1.0, label='echotimer_rx')
    ...
    def set_num_delay_samp(self, num_delay_samp):
        ...
        self.echotimer_rx_compensator_0.num_delay_samps = int(self.num_delay_samp)
