...
class sic_tap_calibration(gr.top_block, Qt.QWidget):

    def __init__(self):
        gr.top_block.__init__(self, "SIC Tap Calibration", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("SIC Tap Calibration")
        ...
        self.settings = Qt.QSettings("gnuradio/flowgraphs", "sic_tap_calibration")
        ...
        self._sic_cal_record_callback = lambda i: Qt.QMetaObject.invokeMethod(_sic_cal_record_check_box, "setChecked", Qt.Q_ARG("bool", self._sic_cal_record_choices_inv[i]))
        self._sic_cal_record_callback(self.sic_cal_record)
        _sic_cal_record_check_box.stateChanged.connect(lambda i: self.set_sic_cal_record(self._sic_cal_record_choices[bool(i)]))
        self.top_grid_layout.addWidget(_sic_cal_record_check_box, 1, 5, 1, 1)
        for r in range(1, 2):
...
def main(top_block_cls=sic_tap_calibration, options=None):
