...
本文档描述测试流图 `usrp_ofdm_single_bs_range` 的端到端信号处理：Style1 定时突发发射、同 epoch 定点收样、Torch CUDA 距离谱，以及内嵌 Range Profile / MUSIC。

对应文件：

- [`usrp_ofdm_single_bs_range.grc`](usrp_ofdm_single_bs_range.grc) / [`usrp_ofdm_single_bs_range.py`](usrp_ofdm_single_bs_range.py)
...
本流图实现**零多普勒 OFDM 单站雷达**（环回 / 近距目标），**ch0 单发 / 单收**，OFDM 为 **2048 子载波 × 120 kHz → 245.76 MHz**（对齐 **X4_200** + 单条 10GbE）：

1. **启动时**在 CUDA 上一次性预计算一整 CPI 的频域网格与时域 IQ，拷回 Host。
2. **运行期** Host 仅 memcpy 重放时域突发，打 Style1 时间标签；`out0` 接到 Sink 流口 0；并向 USRP Source **异步**下发 `NUM_SAMPS_AND_DONE`。
3. **同 `tx_epoch`** 时刻 ch0 发射与接收各 `8704` 个样点。
4. **`OfdmBurstRx`（`num_channels=1`）**找 `rx_time`/拼 CPI/delay，CUDA 距离谱，单窗 Range Profile。

核心块：`OfdmBurstTxSource`（单口）+ 单输入 `OfdmBurstRx`；UHD `channels=[0]`。
...
flowchart LR
  ...
  subgraph rxHost [Host_RX]
    Rx1["OfdmBurstRx_nchan1"]
  end

  TxSrc -->|"out0"| Sink
  TxSrc -->|"msg tx_freq_cpi / tx_schedule"| Rx1
  Source -->|"port0"| Rx1
...
| 源                                | 目的                          | 类型                                         |
| --------------------------------- | ----------------------------- | -------------------------------------------- |
| `OfdmBurstTxSource.out0`        | `uhd_usrp_sink_0:0`         | 突发 IQ（Style1 标签）                       |
| `OfdmBurstTxSource.tx_freq_cpi` | `ofdm_burst_rx_0`           | PMT（首 CPI 一次；粘性复用）                 |
| `OfdmBurstTxSource.tx_schedule` | `ofdm_burst_rx_0`           | PMT（每 CPI）                                |
| `uhd_usrp_source_0:0`           | `ofdm_burst_rx_0:0`         | 单路接收 IQ                                  |

**RF / 通道约定**：

| 项 | 值 |
|----|-----|
| UHD `channels` / `nchan` | `[0]` / `1` |
| Sink 天线 | `TX/RX` |
| Source 天线 | `RX1` |
| `freq` / `TX_gain` / `RX_gain` | ch0 |
| RX 块 | `num_channels=1` |

**反压 / 吞吐说明**：

1. TX / Sink / Source / RX 均为单流口。
...
| `TX_gain` / `RX_gain`           | 20                                        | USRP 增益（GUI；ch0）                           |
...
| 输出              | 形状 / 类型                         | 下游                  |
| ----------------- | ----------------------------------- | --------------------- |
| out0              | `complex64` 标量流，CPI=8704      | Sink:0                |
...
### 7.2 `OfdmBurstRxBlock`（`num_channels=1`，单窗 `Range Profile`）

`out_sig=None`：无流输出。找 `rx_time`、拼 CPI、delay 后异步 CUDA DSP；内嵌一个 `_RangeProfileDisplay`。

| 步骤 | 方法                     | 做什么                                                                                 | 设备            | 数据                   |
| ---- | ------------------------ | -------------------------------------------------------------------------------------- | --------------- | ---------------------- |
| 1    | `_on_tx_freq_cpi`      | 解析一次 → `_tx_cpi_latest`（粘性复用）                                               | Host            | TX 参考 CPI            |
| 2    | `_on_tx_schedule`      | 收到计划 epoch（轻量 ack）                                                            | Host            | msg                    |
| 3    | `_ingest_iq_ch`        | 找`rx_time` → memcpy 满 `burst_len`                                                   | Host            | `(8704,)`              |
| 4    | `_apply_delay`         | 左移`num_delay_samp`（默认 277）                                                      | Host            | 原地                   |
| 5    | `_try_flush_ready`     | 就绪 → 入 DSP 队列                                                                    | Host            | CPI                    |
| 6    | 异步 `process_rx_cpi_torch` | `OFDMDemodulator`+√N + 距离谱（不阻塞 GR `work`） | **CUDA** | 见下 |
| 7    | `_emit_plot_and_music` | ROI 刷新；可选 MUSIC | Host / Qt + CPU | — |

### 7.3 CUDA 距离谱（`process_rx_cpi_torch`）

1. 输入时域 `(8704,)`（内部可按 `(1, 8704)` 批路径处理）。
2. `OFDMDemodulator`（`l_min=0`）+ ×√`fft_len` → `(1, 4, 2048)`。
3. 共享 `tx_freq` broadcast：`H = tx / rx`。
4. 零填充到 `8192`，× Blackman–Harris 窗。
5. 距离维 FFT；功率跨符号求和 → dB / 复数谱 `(1, 8192)`。
...
t ≈ epoch − 0.5 s（首帧）/ epoch − 几十 ms（后续）
  ...
  Host memcpy out0 → USRP Sink DMA（ch0）

t = epoch
  USRP TX ch0 发 8704 samp（~35.4 µs @ 245.76 MHz）
  USRP RX ch0 同刻收 8704 samp（NUM_SAMPS_AND_DONE）
...
与协作采集类似的会话语义；落盘内容为本流图单通道复数距离谱 `cx`（文件名仍用 `divide_profiles_NNN`）。

| 项 | 说明 |
| --- | --- |
| GUI | **Record Enable**；**Target range (m)**；写满后自动停并取消勾选 |
| 帧数 | 每次固定 **`record_max_frames=55`**（约 `idle_ms=20` 时 ~1.1 s） |
| 目录 | `data/usrp_ofdm_single_bs_range/`（`repo_data_dir`） |
| 会话文件 | 递增 `divide_profiles_001`、`002`…（不覆盖旧文件） |
| 元数据 | 同目录 `target_positions.csv`：`recorded_at_utc,target_range_m,data_file,record_max_frames` |
| 内容 | 相干积分复数距离谱 `cx`（非 dB） |
| 帧布局 | 每 CPI 写入 `(1, 8192)` `complex64`；读回 `reshape(-1, 1, 8192)` 或 `(-1, 8192)` |
| 读回 | `np.fromfile(path, dtype=np.complex64).reshape(-1, 8192)` |

安装钩子：`install_usrp_ofdm_1ch_record_flow`（分配路径 + 写真实距离 CSV + 录满停）。

---

## 11. （已移除）多通道相位校准与 AoA

本流图现为 **1TX/1RX**，不再提供 Phase Cal / 多路 phase bias / AoA GUI。库内 `OfdmBurstRxBlock` 仍保留多通道 API，供其它流图使用。
...
| 模块              | 路径                                                    | 角色                                                 |
| ----------------- | ------------------------------------------------------- | ---------------------------------------------------- |
| Flowgraph         | `usrp_ofdm_single_bs_range.py` / `.grc`              | 拓扑与参数                                           |
...
| Style1 / 调度消息 | `src/isac_imp/burst_pack.py`                          | `tx_sob`/`tx_time`/`tx_eob`、`tx_freq_cpi`/`tx_schedule` |
