#!/usr/bin/env python3
"""几何残差头（S2-geom）A/B/C 对照矩阵。

在新默认超参下训练三组：baseline / geom 端到端 / geom+stopgrad，
逐项 Run2 全帧评测，汇总 Global/Inner/Outer RMSE 到 CSV。

示例::

    python script/experiment/run_cnn_geom_residual_matrix.py
    python script/experiment/run_cnn_geom_residual_matrix.py --only geom_residual
    python script/experiment/run_cnn_geom_residual_matrix.py --skip-train
"""
...
SUMMARY_CSV = PROJECT_ROOT / "out/cooperative_monostatic/cnn_geom_residual_summary.csv"
...
class Experiment:
    ...
    def train_cli_args(self) -> list[str]:
        ...
        if self.stopgrad_geom:
            args.append("--stopgrad-geom")
        else:
            args.append("--no-stopgrad-geom")
    ...
    def meta_row(self) -> dict[str, str | float | int]:
        return {
            ...
            "stopgrad_geom": int(self.stopgrad_geom),
        }


EXPERIMENTS: tuple[Experiment, ...] = (
    Experiment(
        exp_id="baseline_new_defaults",
        description="new defaults, no geom residual",
    ),
    Experiment(
        exp_id="geom_residual",
        description="geom residual end-to-end (no stopgrad)",
        geom_residual=True,
    ),
    Experiment(
        exp_id="geom_stopgrad",
        description="geom residual + stopgrad_geom",
        geom_residual=True,
        stopgrad_geom=True,
    ),
)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="S2-geom residual A/B/C matrix")
    parser.add_argument(
        "--skip-train",
        action="store_true",
