...
from .channel_components import build_channel
from .ofdm_components import build_ofdm_components
from .rt_scene_components import build_rt_scene
from .sensing_components import build_sensing_components
...
class SystemComponents:
    ...
    """OFDM 解调器（由 ``build_ofdm_components`` 按 ``SystemParams.ofdm`` 构造，含 ``l_min`` 等）。"""
    ...
    """CFAR 检测器实例（由 ``from_system_params`` 按 ``SystemParams.sensing.cfar`` 构造）。"""
    music_estimator: MUSICEstimator = field(default_factory=MUSICEstimator)
    """MUSIC估计器"""
    zc_source: Optional[ZCSource] = None
    """Zadoff-Chu 源（仅 ``sensing.source.type == 'zc'`` 时由 ``build_sensing_components`` 构造）。"""
    ...
    def from_system_params(
        ...
        """从系统参数创建系统组件"""
        ofdm = build_ofdm_components(system_params, device=device)
        rt_scene = build_rt_scene(system_params, resource_grid=ofdm.rg)
        chan = build_channel(ofdm.rg, rt_scene)
        sens = build_sensing_components(system_params, ofdm.rg, device=device)
