...
def _validate_geometry(
    ...
    *,
    ...
    if (n_sym, fft_size, cp, scs) != (num_symbols, fft_len, cp_len, subcarrier_spacing):
        raise ValueError(
            f"GRC OFDM 参数与 TOML 不一致: "
            f"GRC=({num_symbols},{fft_len},{cp_len},{subcarrier_spacing}) "
...
class EchotimerTransmitCacheBlock(gr.basic_block):
    ...
    - out1：vlen=fft_len 向量，CPI 首符号打 ``length_tag_key``，值为 ``num_symbols``
    """
    ...
    def __init__(
        ...
        self._burst_len_samples = self._num_symbols * self._sym_samples
        ...
        self.set_min_output_buffer(max(self._burst_len_samples * 2, self._num_symbols * 2))
    ...
    def _load_buffers(self) -> None:
        ...
        if freq.shape[0] != self._num_symbols:
            raise ValueError(
                f"x_rg.npy 符号数 {freq.shape[0]} != num_symbols {self._num_symbols}"
            )
        ...
        self._log(
            ...
            f"symbols={self._num_symbols} fft_len={self._fft_len}"
        )
