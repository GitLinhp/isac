#!/usr/bin/env python3
"""离线预生成发射波形缓存目录（三个 .npy），供 simulator_ofdm GRC 使用。
...
示例::
    ...
    python example/simulator_ofdm/generate_transmit_cache.py --device cpu --force
"""
...
def main() -> None:
    ...
    paths = cache.npy_paths()
    if args.force:
        removed = cache.remove_files()
        if removed:
            print(f"已删除旧缓存: {cache.cache_dir} ({', '.join(removed)})")

    existed = cache.is_complete()
