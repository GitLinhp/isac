...
| `antenna_rx` | 接收天线 | **`RX1`** |
| `clock_source_tx` / `clock_source_rx` | 参考时钟（10 MHz，REF IN） | **`internal`**；接外部 10 MHz 时 **`external`** |
| `time_source_tx` / `time_source_rx` | 时间 / PPS 源（PPS IN） | **`internal`**；10 MHz + PPS 均接入时 **`external`** |
...
外部时钟配置须与 **REF IN / PPS IN 实际接线** 一致，且 **device args 与 Echotimer 块参数必须相同**（避免 internal 初始化后再 runtime 切换导致 `PLL_SYNC_DONE` 超时）：

| 硬件接线 | `clock_source` | `time_source` |
|---------|----------------|---------------|
| 无外部源 | `internal` | `internal` |
| 仅 10 MHz（REF IN） | `external` | `internal` |
| 10 MHz + PPS（REF IN + PPS IN） | `external` | `external` |

单机外部 10 MHz + PPS（设备 349B642）：

```
type=x4xx,serial=349B642,mgmt_addr=192.168.1.100,addr=192.168.10.2,clock_source=external,time_source=external
```

双机外部 10 MHz + PPS（[`usrp_ofdm_two_devices_echotimer_dd.grc`](../gnuradio/tests/two_base/usrp_ofdm_two_devices_echotimer_dd/usrp_ofdm_two_devices_echotimer_dd.grc)，TX=349B642，RX=33ABFDE）：

```
address1 = type=x4xx,serial=349B642,mgmt_addr=192.168.1.100,addr=192.168.10.2,clock_source=external,time_source=external
address0 = type=x4xx,serial=33ABFDE,mgmt_addr=192.168.1.101,addr=192.168.11.2,clock_source=external,time_source=external
```

> **注意**：`time_source=external` 要求 PPS IN 接入 1 PPS 信号；仅接 10 MHz 而设为 external 会导致 `PLL_SYNC_DONE` / `Sample Clock PLL1 failed to lock` 错误。

> **GRC Generate 会覆盖 `.py`**：在 GRC 中修改 `uhd_dev_args` / `address0` / `address1` 或 Echotimer 时钟参数后，Generate 会重写 Python 文件；若仅手改 `.py`，下次 Generate 会丢失修改。Generate 后请确认 `.py` 中 device args 含 `serial=` 与 `clock_source` / `time_source`。

启动前可用以下命令逐台验证外部参考锁定（两台均需 `ref_locked: True`）：

```bash
uhd_usrp_probe --args="type=x4xx,serial=349B642,mgmt_addr=192.168.1.100,addr=192.168.10.2,clock_source=external,time_source=external"
uhd_usrp_probe --args="type=x4xx,serial=33ABFDE,mgmt_addr=192.168.1.101,addr=192.168.11.2,clock_source=external,time_source=external"
```
